...
# Same lookup as §2.1: flatten the casing, then keep the candidate under this table's anchor.
...
def as_written(table):
    ...
    buf = StringIO()
    format_for_export(globals()[f'{table}_final'], table).to_csv(buf, index=False)
    buf.seek(0)
